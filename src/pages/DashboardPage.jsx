...
function DashboardPage() {
  ...
  const ringkasan = useMemo(() => {
    ...
    const pesananAktif = pesanan.filter((item) => item.status !== 'Selesai').length
    const kurirAktif = kurir.filter((item) => item.aktif).length
    return { totalPendapatan, pesananAktif, kurirAktif, totalPelanggan: pelanggan.length }
  }, [pesanan, kurir, pelanggan.length])

  return (
    <section className="page-section">
      <article className="rounded-2xl border border-emerald-100 bg-white/80 p-4 shadow-sm">
        <h3 className="m-0 text-lg font-semibold text-emerald-900">Akses Cepat</h3>
        <p className="mb-3 mt-1 text-sm text-emerald-700">Navigasi cepat antar halaman utama admin.</p>
        <div className="flex flex-wrap gap-2">
          <Link to="/pesanan" className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-sm text-emerald-800 no-underline">
            Buka Pesanan
          </Link>
          <Link to="/pelanggan" className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-sm text-emerald-800 no-underline">
            Lihat Pelanggan
          </Link>
          <Link to="/laporan" className="rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-sm text-emerald-800 no-underline">
            Buka Laporan
          </Link>
        </div>
      </article>
